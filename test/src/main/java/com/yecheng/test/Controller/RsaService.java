...
public class RsaService {
    ...
    //解密使用自己私钥
    public String doDecryption(ServletInputStream input) throws IOException {
    ...
    public String testEn(String text) throws Exception {
        //读取公钥
        BufferedReader reader = new BufferedReader(new FileReader("/Users/bytedance/myKey.txt"));
        BigInteger n = new BigInteger(reader.readLine());
        BigInteger publicKey = new BigInteger(reader.readLine());
        ...
        return doEncryption(text, publicKey, n);
    }
    ...
    private String doEncryption(String plaint, BigInteger publicKey,  BigInteger n) {
        ...
        for(int i = 0; i < plaint.length(); i = i + 2) {
            ...
            char b = plaint.charAt(i + 1);

            BigInteger m = new BigInteger( ( (((int)a) << 16 ) + b) + "");
